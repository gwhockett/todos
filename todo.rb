...
# Sets new id number for a list item
def next_id_number(list)
  return 0 unless list.map { |item| item[:id] }.max
  list.map { |item| item[:id] }.max + 1
end
...
post "/lists" do
  ...
  id = next_id_number(session[:lists])
...
post "/lists/:list_id/todos/:id/destroy" do
  ...
  if env["HTTP_X_REQUESTED_WITH"] == "XMLHttpRequest"
    ...
    session[:success] = "The list item has been deleted!"
    redirect "/lists/#{@list_id}"
  end
end
